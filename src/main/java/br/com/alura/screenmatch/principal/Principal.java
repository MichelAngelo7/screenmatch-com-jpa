...
public class Principal {
	...
	public void exibeMenu() {
		...
		while (opcao != 0) {
			...
			case 7:
				...
				break;
			case 8:
				buscarSerieTemporadaAvaliacao();
				break;
			case 9:
				buscarEpisodioTrecho();
				break;
			case 10:
				buscarTopCincoEpisodiosSerie();
				break;
			case 11:
				buscarEpisodiosData();
				break;
			case 12:
				buscarSerieAtorAvaliacao();
				break;
			case 0:
	...
	private void buscarTopCincoEpisodiosSerie() {
		buscarSerieTitulo();
		if (serieBuscada.isPresent()) {
			Serie serie = serieBuscada.get();
	...
	private void buscarSerieTemporadaAvaliacao() {
		...
		serieEncontradas.stream()
				.forEach(s -> System.out.println("Nome: " + s.getTitulo() + " Avaliações: " + s.getAvaliacao()));
	...
	private void buscarSerieCategoria() {
		...
		System.out.println("Série por categoria " + nomeGenero);
		serieCategoria.forEach(System.out::println);

	}

	private void buscarTopCincoSerie() {
		List<Serie> serieTop = repositorio.findTop5ByOrderByAvaliacaoDesc();
		serieTop.stream().forEach(s -> System.out.println("Nome:" + s.getTitulo() + "Avaliação: " + s.getAvaliacao()));

	}

	private void buscarSerieAtorAvaliacao() {
		System.out.println("Qual nome para busca:");
		var nomeAtor = leitura.nextLine();
		System.out.println("Informe uma avaliação miníma para pesquisa:");
		var avaliacao = leitura.nextDouble();
		...
		seriesEncontradas.stream()
				.forEach(s -> System.out.println("Nome: " + s.getTitulo() + " Avaliação: " + s.getAvaliacao()));
	}
